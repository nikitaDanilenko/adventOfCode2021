...
private lazy val input: List[Line] = Source
...
@main
def solution2: Unit =
  val allPositions = input.flatMap(l =>
    horizontalPositions(l) ++ verticalPositions(l) ++ diagonalUpPositions(l) ++ diagonalDownPositions(l)
  )
  val markedArea = markPositions(Map.empty, allPositions)
  pprint.log(markedArea.count(_._2 > Natural.one))
